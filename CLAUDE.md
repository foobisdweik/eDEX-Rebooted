...
**Native migration — full SwiftUI + Rust app (`macos/eDEXNative/`).** The migration's chosen path is a standalone Swift app (SwiftPM) that links the Rust core (`crates/edex-core` via the `crates/edex-ffi` UniFFI layer) and replaces the WKWebView frontend panel-by-panel. The earlier **Approach A** per-panel `NSView` slots (`src-tauri/src/native_panels.rs`) and the column-granular `native_mount` clock pilot were an interim bridge proving AppKit geometry/theming; they are **frozen/superseded** by the native app and not where new work goes. The authoritative roadmap and per-phase completion log live in the plan below.

## Conversion docs (authoritative for the migration)

- `docs/plans/full-native-swift-rust-conversion-2026-05-30.md` — **the authoritative plan**: the Phase 0–11 roadmap, the dependency/deletion gates, the per-phase recipe, and the running completion log. **Start here.**
- `docs/plans/ffi-throughput-decision-2026-05-30.md` — the FFI-throughput decision (UniFFI control-plane now; a narrow C ABI reserved for terminal streaming) that feeds Phase 9.
- The native app lives under `macos/eDEXNative/` (per-panel pure `*Support` modules + SwiftUI views); legacy panel behavior is read directly from `src/classes/*.class.js` during each conversion.
