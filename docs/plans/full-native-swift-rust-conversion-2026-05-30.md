...
- **Next (agreed sequencing, 2026-06-02):** Return to **Phase 5.6 toplist** now that 6.2 provides the process-modal container needed to complete the panel and click-through process modal in one pass. There is still no OS-pid kill command; `pty_kill` only closes internal PTYs, so process killing must be explicitly scoped during 5.6.
