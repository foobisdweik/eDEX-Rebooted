...
- **Phase 5.6 — IN PROGRESS.** The Swift app is adding the native TOPLIST panel now that 6.2 provides the custom modal container. Scope is legacy-faithful and display-only: a compact 5-row top-process panel refreshed every 2s, plus a click-through "Active Processes" modal with sortable PID/Name/User/CPU/Memory/State/Started/Runtime columns refreshed every 1s while open. A new typed FFI `toplist_snapshot(collapse_threads_by_name, include_process_list)` uses `SysinfoService::panel_snapshot(..., top_limit=5, ...)`; `ToplistSupport` owns one-decimal percent formatting, legacy CPU/memory score sorting, sort-state cycling, and runtime formatting. No OS-pid kill command is being added in this subphase because `toplist.class.js` is read-only and `pty_kill` only closes internal PTYs.
- **Next:** Finish Phase 5.6 validation/PR, then continue with Phase 6.3 settings editor unless sequencing changes.
